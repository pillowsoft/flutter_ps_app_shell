...
void main() {
  group('Universal Back Navigation Architecture Tests', () {
    test('Back navigation logic is independent of navigation mode', () {
      ...
      expect(shouldShowBackButton('/home', false), false); // Single-level route, no history
      expect(shouldShowBackButton('/navigation/detail/1', false), true); // Nested route, no history
      expect(shouldShowBackButton('/profile/settings', false), true); // Nested route, no history
      expect(shouldShowBackButton('/', true), true); // Root route, has history
      expect(shouldShowBackButton('/home', true), true); // Single-level route, has history
      expect(shouldShowBackButton('/navigation/detail/1', true), true); // Nested route, has history
    });
    ...
    test('Navigation mode detection supports all screen sizes', () {
      ...
      bool useSidebar(double width, int visibleRoutes) => isVeryWideScreen(width);
    ...
    test('Path parsing correctly identifies nested routes', () {
      ...
      expect(getPathSegments('/navigation/detail/1'), ['navigation', 'detail', '1']);
      expect(getPathSegments('/profile/settings/advanced'), ['profile', 'settings', 'advanced']);
    ...
    test('Architecture ensures single source of truth', () {
      ...
      expect(
        OldArchitecture.shouldShowBackButtonInMobileDrawer(testPath, testCanPop),
      ...
      final universalResult = NewArchitecture.shouldShowBackButton(testPath, testCanPop);
